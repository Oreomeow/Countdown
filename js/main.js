...
$.fn.countDown = function (_boolean, _this) {
    ...
    _this.each(function (index, el) {
        ...
        sh[index] = setInterval(function () {
            var times = thisObj.data('times'); // 获得 timeBox 的 data 值，即结束时间
            endtime = new Date(times); // 把 data 值转换成时间
            nowtime = new Date(); // 获得当前时间
            lefttime = parseInt((endtime.getTime() - nowtime.getTime()) / 1000); // 结束时间 - 当前时间得到毫秒数，再除以 1000 得到两个时间相差的秒数
            if (_boolean) {
